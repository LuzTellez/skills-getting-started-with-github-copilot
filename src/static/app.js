    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = details.participants.length
          ? `<div class="participant-tags">${details.participants
              .map((participant) => `<span class="participant-pill" data-activity="${name}" data-email="${participant}"><span>${participant}</span><button class="delete-btn" aria-label="Remove ${participant}">×</button></span>`)
              .join("")}</div>`
      ...
      activitiesList.addEventListener("click", async (event) => {
        if (event.target.classList.contains("delete-btn")) {
          const pill = event.target.closest(".participant-pill");
